...
def main_page():
    ...
    image = Image.open("D:\\Desktop\\1.jpg")
...
def team_page(member_name):
    ...
    if member_name == "张睿":
        ...
        st.markdown('<div class="fade-in">', unsafe_allow_html=True)
        st.image("魏恒.jpg", caption=f"{member_name} 的照片", use_column_width=True)
        st.write(f"{member_name} 是我们团队的设计师。他擅长图形设计和用户体验。")
        st.markdown('</div>', unsafe_allow_html=True)
    elif member_name == "吴子俊":
        st.markdown('<div class="fade-in">', unsafe_allow_html=True)
        st.image("吴子俊.jpg", caption=f"{member_name} 的照片", use_column_width=True)
        st.write(f"{member_name} 是我们团队的开发者。他对新技术充满热情。")
        st.markdown('</div>', unsafe_allow_html=True)
    elif member_name == "李冠承":
        st.markdown('<div class="fade-in">', unsafe_allow_html=True)
        st.image("李冠承.jpg", caption=f"{member_name} 的照片", use_column_width=True)
        st.write(f"{member_name} 是我们团队的设计师。他擅长图形设计和用户体验。")
        st.markdown('</div>', unsafe_allow_html=True)
    elif member_name == "吴灿":
        st.markdown('<div class="fade-in">', unsafe_allow_html=True)
        st.image("吴灿.jpg", caption=f"{member_name} 的照片", use_column_width=True)
        st.write(f"{member_name} 是我们团队的设计师。他擅长图形设计和用户体验。")
        st.markdown('</div>', unsafe_allow_html=True)
